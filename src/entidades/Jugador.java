...
public class Jugador extends Entidad {
	/**
	 * Atributos del jugador: - Controlan animación, movimiento, físicas (salto,
	 * gravedad), barras de estado y colisiones. - Incluyen referencias a recursos
	 * gráficos, estado del juego, y propiedades de combate.
	 */
...
//    private float yDrawOffset = 4 * Juego.SCALE;
	...
	/**
	 * Constructor del jugador. Inicializa dimensiones del sprite, estado inicial,
	 * recursos gráficos, animaciones y cajas de colisión.
	 * 
	 * @param personajeJugable Objeto con datos del personaje actual (sprite,
	 *                         hitbox, animaciones).
	 * @param jugando          Referencia al estado actual del juego.
	 */
	public Jugador(PersonajeJugable personajeJugable, Jugando jugando, int jugadorID) {
	 ...
	 * Controla animaciones, físicas, ataques, vida, interacción con el entorno y
	 * efectos de estado. - Cambia al estado MUERTO cuando la vida llega a cero y
	 * ejecuta las animaciones finales.
	 */
	public void actualizar() {
		...
		if (vidaActual <= 0) {
			...
			} else if (indiceAnimacion == personajeJugable.getSpriteAmount(MUERTO) - 1
					&& contadorAnimacion >= ANI_VELOCIDAD - 1) {
				jugando.getGame().getAudioPlayer().reproducirEfecto(AudioJugador.GAMEOVER);
			} else {
				actualizarAnimacionTick();

				if (enAire)
					if (PuedeMoverAqui(cajaColision.x, cajaColision.y + velocidadAire, cajaColision.width,
							cajaColision.height, lvlData)) {
	...
	/**
	 * Controla el avance de animaciones del jugador. Cambia de frame en función de
	 * un contador y reinicia la animación si es necesario. También reinicia estado
	 * de ataque y golpe si se ha completado la animación correspondiente.
	 */
	...
	/**
	 * Actualiza la posición de la caja de ataque en función de la dirección del
	 * jugador. Si el jugador se mueve o activa un ataque de poder, la caja se
	...
	/**
	 * Recalcula el ancho visual de la barra de poder. El poder se regenera
	 * automáticamente con el tiempo, incrementando su valor cada cierto número de
	 * frames.
	 */
	...
	/**
	 * Recalcula el ancho visual de la barra de vida basado en la vida actual. Esto
	 * se utiliza para renderizar la barra de forma proporcional.
	 */
	...
	/**
	 * Calcula el desplazamiento horizontal y vertical del jugador. Incluye físicas
	 * de salto y caída, verificación de colisiones con el entorno y actualización
	 * de posición.
	 */
	private void actualizarPos() {
		...
		if (enAire && !atqueDePoderActivo) {
			if (PuedeMoverAqui(cajaColision.x, cajaColision.y + velocidadAire, cajaColision.width, cajaColision.height,
					lvlData)) {
				cajaColision.y += velocidadAire;
				velocidadAire += GRAVEDAD;
	...
	/**
	 * Aplica el movimiento horizontal al jugador. Si detecta una colisión, ajusta
	 * la posición y cancela el ataque de poder si estaba activo.
	 */
	private void actualizarXPos(float xVelocidad) {
		if (PuedeMoverAqui(cajaColision.x + xVelocidad, cajaColision.y, cajaColision.width, cajaColision.height,
				lvlData))
	...
	/**
	 * Modifica la vida del jugador. Si el valor es negativo y el jugador no está ya
	 * en estado de golpe (HIT), cambia a dicho estado. La vida se mantiene entre 0
	...
	/**
	 * Variante de cambiarVida que también calcula la dirección del retroceso en
	 * función de la posición del enemigo.
	 */
	...
	/**
	 * Aumenta o reduce el poder del jugador. El valor se limita entre 0 y
	 * poderMaxValor.
	 */
	...
	/**
	 * Verifica si el jugador ha ejecutado un ataque y detecta colisiones con
	 * enemigos u objetos rompibles. Reproduce el sonido del ataque y aplica daño si
	 * corresponde.
	 */
	private void checkAtaque() {
		...
		jugando.checkEnemyHit(cajaAtaque, this.jugadorID);
		jugando.checkObjectHit(cajaAtaque);
		jugando.getGame().getAudioPlayer().reproducirSonidoDeAtaque();
	}

	/**
	 * Verifica si el jugador está dentro de una celda de agua. Si está en el agua,
	 * se considera una muerte instantánea.
	 */
	...
	/**
	 * Verifica si el jugador ha tocado un objeto recogible como pociones. Notifica
	 * al controlador de nivel que se recolectó el objeto.
	 */
	...
	/**
	 * Verifica si el jugador ha tocado una trampa puas. Aplica efectos definidos
	 * por el nivel actual.
	 */
	...
	/**
	 * Dibuja la barra de vida (roja) y la barra de poder (amarilla) sobre la
	...
	private void dibujarUI(Graphics g) {
		...
		// Background UI
		g.drawImage(imgBarraDeEstado, barraDeEstadoX, barraDeEstadoY + offsetY, anchoBarraDeEstado, altoBarraDeEstado,
				null);

		// Health bar
		g.setColor(Color.red);
		g.fillRect(barraDeVidaXInicio + barraDeEstadoX, barraDeVidaYInicio + barraDeEstadoY + offsetY, anchoVida,
				altoBarraDeVida);

		// Power bar
		g.setColor(Color.yellow);
		g.fillRect(barraDePoderXInicio + barraDeEstadoX, barraDePoderYInicio + barraDeEstadoY + offsetY, anchoPoder,
				altoBarraDePoder);
	}
	...
	/**
	 * Crea y posiciona la caja de colisión del ataque del jugador. La caja se
	 * reposiciona dinámicamente en cada frame según la dirección.
	 */
	...
	/**
	 * Carga la matriz de datos del nivel, usada para verificar colisiones con el
	 * entorno. Si el jugador no está tocando el piso, entra en estado de caída.
	 */
	...
	/**
	 * Inicia un ataque de poder si se tiene suficiente energía (mínimo 60
	 * unidades). Reduce el poder actual y activa el estado de ataque especial.
	 */
	...
	/**
	 * Renderiza al jugador en pantalla con su animación correspondiente. También
	 * dibuja su interfaz de usuario (barras de vida y poder).
	 * 
	 * @param g         Objeto gráfico de Java 2D.
	 * @param lvlOffset Offset horizontal del nivel (para scroll).
	 */
	public void render(Graphics g, int lvlOffset) {
		g.drawImage(animacion[personajeJugable.getRowIndex(estado)][indiceAnimacion],
				(int) (cajaColision.x - personajeJugable.xDrawOffset) - lvlOffset + giroX,
				(int) (cajaColision.y - personajeJugable.yDrawOffset + (int) (desplazamientoRetroceso)), ancho * giroW,
				alto, null);
...
//		dibujarCajaAtaque(g, lvlOffset);
	...
	/**
	 * Reinicia por completo el estado del jugador: vida, poder, dirección, físicas,
	 * cajas de colisión. Se usa al reaparecer o al reiniciar el juego.
	 */
	...
	/**
	 * Reinicia el contador y el índice de animación a cero. Usado al cambiar de
	 * estado visual.
	 */
	...
	/**
	 * Inicia el salto del jugador si no está en el aire. Establece velocidad
	 * vertical negativa y activa el estado enAire.
	 */
	private void saltar() {
		if (enAire)
			return;
	...
	/**
	 * Establece el estado de animación del jugador (idle, corriendo, saltando,
	 * cayendo, atacando). Basado en la entrada del jugador y su estado físico (en
	 * el aire, atacando, etc).
	 */
