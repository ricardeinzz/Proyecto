...
/**
 * Clase base abstracta para todos los enemigos del juego. Hereda comportamiento
 * general de la clase Entidad.
 */
public abstract class Enemigos extends Entidad {
	...
	// Celda vertical donde se encuentra el enemigo (para ver si el jugador está a
	// la misma altura)
	...
	private boolean esperandoParaDesaparecer = false;
...
//		return false;
	...
	protected void checkPlayerHit(Rectangle2D.Float cajaAtaque, Jugador jugador, Jugador jugador2) {
		...
		if (!ataqueChecked) {
		    ...
		    if (cajaAtaque.intersects(jugador2.cajaColision)) {
	...
	// Calcula cuánto se debe mover la imagen del enemigo hacia la izquierda si está
	// volteado
	public int flipX() {
	...
	public void hurt(int cantidad) {
		hurt(cantidad, 0); // jugadorID 0 = muerte por ambiente, trampa, etc.
	}

	public void hurt(int cantidad, int jugadorID) {
		if (estado == MUERTO)
		...
		if (vidaActual <= 0) {
			...
			estado = MUERTO;
			muerto = true;
			jugando.registrarKill(jugadorID, this.getClass().getSimpleName());
	...
	// Comportamiento del enemigo cuando está en el aire (caída, daño por agua,
	// etc.)
	...
	protected boolean isPlayerCloseForAttack(Jugador jugador, Jugador jugador2) {
		Jugador objetivo = getJugadorMasCercano(jugador, jugador2);
		float distanciaX = Math.abs(objetivo.cajaColision.x - cajaColision.x);

		return switch (tipoEnemigo) {
		case CANGREJO -> distanciaX <= distanciaAtaque;
		case TIBURON -> distanciaX <= distanciaAtaque * 2;
		case ESTRELLA -> distanciaX <= distanciaAtaque;
		default -> false;
		};
	}
	...
	// Mueve al enemigo en la dirección actual, o cambia de dirección si no puede
	// continuar
	...
	// Restablece al enemigo a su estado inicial (por ejemplo, tras morir y
	// reiniciar el nivel)
...
//			dirCaminar = IZQUIERDA;
	...
	protected void updateAnimationTick() {
		...
		if (contadorAnimacion >= ANI_VELOCIDAD) {
			...
			int framesTotales = GetSpriteAmount(tipoEnemigo, estado);

			if (indiceAnimacion >= framesTotales) {
				if (estado == MUERTO) {
					if (esperandoParaDesaparecer) {
						activo = false;
					} else {
						indiceAnimacion = framesTotales - 1;
						esperandoParaDesaparecer = true;
					}
				} else {
					indiceAnimacion = 0;
					if (estado == HIT || estado == ATACANDO)
						estado = IDLE;
				}
			}
		}
