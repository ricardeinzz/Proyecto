...
/**
 * Clase encargada de gestionar todos los enemigos del nivel actual. Controla la
 * carga, actualización, dibujo y colisiones de enemigos como Cangrejo, Estrella
 * y Tiburón.
 */
public class EnemyManager {
	...
	// Constructor: se guarda el estado Jugando y se cargan las imágenes de los
	// enemigos.
	...
	public void actualizar(int[][] lvlData) {
	    ...
	    // Actualiza cada cangrejo si está activo y no está muerto.
	    for (Cangrejo c : nivelActual.getCrabs())
	        if (c.isActivo() && !c.estaMuerto()) {
	            c.actualizar(lvlData, jugando);
	            isAnyActive = true;
	        }

	    // Actualiza cada estrella si está activa y no está muerta.
	    for (Estrella p : nivelActual.getPinkstars())
	        if (p.isActivo() && !p.estaMuerto()) {
	            p.actualizar(lvlData, jugando);
	            isAnyActive = true;
	        }

	    // Actualiza cada tiburón si está activo y no está muerto.
	    for (Tiburon s : nivelActual.getSharks())
	        if (s.isActivo() && !s.estaMuerto()) {
	            s.actualizar(lvlData, jugando);
	            isAnyActive = true;
	        }

	    // Si no quedan enemigos activos y vivos, se marca el nivel como completado.
	    if (!isAnyActive)
	...
	public void cargarEnemigos(Nivel nivel) {
	    ...
	    for (Cangrejo c : nivel.getCrabs()) {
	        c.setJugando(jugando);
	    }
	    for (Estrella e : nivel.getPinkstars()) {
	        e.setJugando(jugando);
	    }
	    for (Tiburon t : nivel.getSharks()) {
	        t.setJugando(jugando);
	    }
	}
	...
	private void cargarImgEnemigos() {
		cangrejoArr = getImgArr(LoadSave.GetSpriteAtlas(LoadSave.CANGREJO_SPRITE), 9, 5, CANGREJO_ANCHO_DEFAULT,
				CANGREJO_ALTO_DEFAULT);
		estrellaArr = getImgArr(LoadSave.GetSpriteAtlas(LoadSave.ESTRELLA_ATLAS), 8, 5, ESTRELLA_ANCHO_DEFAULT,
				ESTRELLA_ALTO_DEFAULT);
		tiburonArr = getImgArr(LoadSave.GetSpriteAtlas(LoadSave.TIBURON_ATLAS), 8, 5, TIBURON_ANCHO_DEFAULT,
				TIBURON_ALTO_DEFAULT);
	}

	// Verifica si el jugador golpeó a algún enemigo mediante su caja de ataque.
	public void checkHitEnemigo(Rectangle2D.Float cajaAtaque, int jugadorID) {
	    for (Cangrejo c : nivelActual.getCrabs())
	        if (c.isActivo() && c.getState() != MUERTO && c.getState() != HIT)
	            if (cajaAtaque.intersects(c.obtenerCajaColision())) {
	                c.hurt(20, jugadorID); // ✅ Aquí se pasa el jugadorID
	                return;
	            }
	
	    // Estrellas
	    for (Estrella p : nivelActual.getPinkstars())
	        if (p.isActivo() && p.getState() != MUERTO && p.getState() != HIT)
	    ...
	    // Tiburones
	    for (Tiburon s : nivelActual.getSharks())
	        if (s.isActivo() && s.getState() != MUERTO && s.getState() != HIT)
	...
	private void dibujarCangrejo(Graphics g, int xLvlOffset) {
		for (Cangrejo c : nivelActual.getCrabs())
			if (c.isActivo()) {
				g.drawImage(cangrejoArr[c.getState()][c.obtenerIndiceAnimacion()],
						...
						CANGREJO_ANCHO * c.flipW(), CANGREJO_ALTO, null);
...
private void dibujarEstrella(Graphics g, int xLvlOffset) {
	for (Estrella p : nivelActual.getPinkstars())
		if (p.isActivo()) {
			g.drawImage(estrellaArr[p.getState()][p.obtenerIndiceAnimacion()],
					...
					ESTRELLA_ANCHO * p.flipW(), ESTRELLA_ALTO, null);
	...
	private void dibujarTiburon(Graphics g, int xLvlOffset) {
		for (Tiburon s : nivelActual.getSharks())
			if (s.isActivo()) {
				g.drawImage(tiburonArr[s.getState()][s.obtenerIndiceAnimacion()],
						...
						TIBURON_ANCHO * s.flipW(), TIBURON_ALTO, null);
				// s.dibujarCajaColision(g, xLvlOffset);
				// s.dibujarCajaAtaque(g, xLvlOffset);
			}
	}

	// Convierte una hoja de sprites en una matriz de imágenes individuales por
	// animación.
	...
	// Reinicia el estado de todos los enemigos (como si el nivel comenzara de
	// nuevo).
	public void reiniciarEnemigos() {
	    ...
	    for (Estrella p : nivelActual.getPinkstars()) {
	        p.resetEnemy();
	        p.setJugando(jugando); 
	    }
	    for (Tiburon s : nivelActual.getSharks()) {
	        s.resetEnemy();
	        s.setJugando(jugando); 
	    }
	}
